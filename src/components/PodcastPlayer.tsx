...
type InputTargetEvent = Event & {
  currentTarget: HTMLInputElement;
};

type ScrubStartEvent = (PointerEvent | TouchEvent) & {
...
export function PodcastPlayer({ episodes, onClose }: PodcastPlayerProps) {
  ...
  const seekToTime = useCallback((rawValue: number) => {
    ...
    trackUmami('podcast-seek', {
      source: 'player',
      slug: currentEpisode?.slug ?? '',
      from: Math.round(clampedProgress),
      to: Math.round(time),
    });
  }, [currentEpisode?.slug]);
  ...
  const togglePlaybackRate = useCallback(() => {
    ...
    trackUmami('podcast-rate-change', {
      source: 'player',
      slug: currentEpisode?.slug ?? '',
      rate: nextRate,
    });
  }, [state.playbackRate, activePlaybackLang, currentEpisode?.slug]);
  
  const handleClose = useCallback(() => {
    persistProgressSnapshot();
    trackUmami('podcast-close', {
      source: 'player',
      slug: currentEpisode?.slug ?? '',
    });
    // 先暂停音频
  ...
  const toggleExpanded = useCallback(() => {
    setIsExpanded(prev => {
      const newVal = !prev;
      trackUmami(newVal ? 'podcast-list-open' : 'podcast-list-close', {
        source: 'player',
        slug: currentEpisode?.slug ?? '',
      });
      return newVal;
  ...
  const progressLabel = activePlaybackLang === 'en' ? 'Playback progress' : '播放进度';
  const rateLabel = activePlaybackLang === 'en' ? `Playback speed ${state.playbackRate}x` : `播放速度 ${state.playbackRate} 倍`;
  const expandCollapseLabel = isExpanded
    ? (activePlaybackLang === 'en' ? 'Collapse playlist' : '收起')
    : (activePlaybackLang === 'en' ? 'Expand playlist' : '展开');
  const closePlayerLabel = activePlaybackLang === 'en' ? 'Close player' : '关闭播放器';
  const retryLabel = activePlaybackLang === 'en' ? 'Retry' : '重试';
    ...
    >
        ...
        <div className="player-controls">
          <button
            ...
            title={activePlaybackLang === 'en' ? 'Toggle playback speed' : '切换播放速度'}
      ...
      {error && (
        <div className="player-error" role="alert" aria-live="assertive" aria-atomic="true">
          ...
          >
            {activePlaybackLang === 'en' ? 'Retry' : '重试'}
          </button>
          <button
