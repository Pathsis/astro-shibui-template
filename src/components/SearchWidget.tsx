...
const classes = {
  ...
  visible: "visible",
  hiddenForHint: "hiddenForHint",
  keyboardHint: "keyboardHint",
  inlineHintGroup: "inlineHintGroup",
  keycap: "keycap",
...
const handleMouseMove = () => {
  ...
  return (
    <div 
      ref={widgetRef}
      class={`${classes.searchWidget} ${showResults ? classes.hasResults : ""}`}
    >
      <div class={classes.searchBox}>
        ...
        <input
          ...
          autocomplete="off"
        />
        <span class={classes.searchIcon} aria-hidden="true">
        ...
        <button
          onClick={clearSearch}
          class={`${classes.clearSearch} ${query ? classes.visible : ""} ${(showNavigationHint || showOpenHint) ? classes.hiddenForHint : ""}`}
        ...
        <div
          class={`${classes.keyboardHint} ${(showNavigationHint || showOpenHint) ? classes.visible : ""}`}
          aria-hidden="true"
        >
          {showNavigationHint ? (
            <>
              <span class={classes.inlineHintGroup}>
                <span class={classes.keycap}>↑</span>
                <span class={classes.keycap}>↓</span>
                <span class={classes.inlineHintText}>{selectHintText}</span>
              </span>
              <span class={classes.inlineHintGroup}>
                <span class={classes.keycap}>Enter</span>
                <span class={classes.inlineHintText}>{openHintText}</span>
              </span>
              <span class={classes.inlineHintGroup}>
                <span class={classes.keycap}>Esc</span>
                <span class={classes.inlineHintText}>{backHintText}</span>
              </span>
            </>
          ) : (
            <>
              <span class={classes.keycap}>{shortcutPrimaryKey}</span>
              <span class={classes.keycap}>K</span>
            </>
          )}
        </div>
      </div>
      ...
      <div 
        ref={resultsRef}
        class={`${classes.searchResults} ${showResults ? classes.visible : ""}`}
        role="listbox"
        aria-live="polite"
        onMouseMove={handleMouseMove}
      >
        {isLoading ? (
          <div class={`${classes.loading} ${classes.visible}`}>{searchingText}</div>
        ) : results.length > 0 ? (
          results.map((hit, index) => (
            <div
              key={hit.objectID}
              class={`${classes.searchResultItem} ${index === focusedIndex ? classes.focused : ""}`}
              role="option"
              tabindex={0}
              data-index={index}
              ...
              >
                <h1
                  dangerouslySetInnerHTML={{
                    __html: normalizeHighlightHtml(
                      hit._highlightResult?.title?.value || escapeHtml(hit.title),
                      query,
                    ),
                  }}
                />
                <p
                  dangerouslySetInnerHTML={{
                    __html: normalizeHighlightHtml(hit._snippetResult?.content?.value || "", query),
                  }}
                />
              </a>
            </div>
...
function isContinuousCjkQuery(query: string): boolean {
  ...
  return trimmed.length > 1 && /^[\u3400-\u9FFF\uF900-\uFAFF]+$/.test(trimmed);
}
